...
def follow(userid, messageid):
    ...
    user = session.query(db.User).filter(db.User.id == userid).first()
    message = session.query(db.Message).filter(db.Message.id == messageid).first()
    print 'follow %s to %s' % (user.name, message.date)
    follower = db.Follower(user.name, datetime.datetime.today())
...
def getOwnMessages(userid):
    session = createSession()
    user = session.query(db.User).filter(db.User.id == userid).first()
    messages = user.ownmessages
    res = []
    for m in messages:
        f = getFollowers(session, m.id)
        if not f: f=[]
        res.append({
            'id': m.id,
            'title' : m.title,
...
if __name__ == '__main__':
    ...
    for m in messages:
        print getFollowers(session, m.id)
